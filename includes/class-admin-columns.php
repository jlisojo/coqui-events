...
class Simple_Events_Admin_Columns {
    ...
    public function set_columns($columns) {
        ...
        foreach ($columns as $key => $title) {
            ...
            if ($key === 'title') {
                $new_columns['se_event_date']     = __('Event Date', 'coqui-events');
                $new_columns['se_event_location'] = __('Location', 'coqui-events');
                $new_columns['se_event_price']    = __('Price', 'coqui-events');
                $new_columns['se_event_status']   = __('Status', 'coqui-events');
            }
        }
    ...
    public function render_columns($column, $post_id) {
        ...
        switch ($column) {
            ...
            case 'se_event_price':
                if ($event['is_free']) {
                    echo '<span class="se-status se-status--free">' . esc_html__('FREE', 'coqui-events') . '</span>';
                } elseif ($event['price']) {
            ...
            case 'se_event_status':
                ...
                if ($event_day < $today) {
                    echo '<span class="se-status se-status--past">' . esc_html__('Past', 'coqui-events') . '</span>';
                } elseif ($event_day === $today) {
                    echo '<span class="se-status se-status--today">' . esc_html__('Today', 'coqui-events') . '</span>';
                } else {
                    ...
                    if ($days_until <= 7) {
                        printf(
                            ...
                            esc_html(sprintf(
                                ...
                                _n('In %d day', 'In %d days', $days_until, 'coqui-events'),
                                $days_until
                            ))
                        );
                    } else {
                        echo '<span class="se-status se-status--upcoming">' . esc_html__('Upcoming', 'coqui-events') . '</span>';
                    }
                }
                break;
        }
